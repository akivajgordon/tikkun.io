...
const ParshaResult = ({ match, item, }) => htmlToElement(`
  <div data-target-class="parsha-result" data-idx="${item.idx}" data-token="${item.token}" data-key="${item.key}">
    <p class="search-result-tag mod-hebrew" data-target-class="result-hebrew">${match.index === 0
    ? decorateString({
        string: item.he,
    ...
    <p class="search-result-tag">${match.index === 1
    ? decorateString({
        string: item.en,
        atIndexes: match.indexes,
        withDecoration: strongify,
...
`);
export { NoResults };
//# sourceMappingURL=ParshaResult.js.map