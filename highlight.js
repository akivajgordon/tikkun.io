...
export const watchForHighlighting = () => onHighlightIntentComplete(book, (selection) => {
    const isLine = (node) => 'dataset' in node && node.dataset.class === 'line';
    const isPage = (node) => {
        if (!('classList' in node))
            return false;
        return [...node.classList].includes('tikkun-page');
    };
    const lineOf = (node) => ancestorOf(node, { matching: isLine });
    const pageOf = (node) => ancestorOf(node, { matching: isPage });
    ...
    const allLines = [
        ...startLine.parentElement.querySelectorAll('[data-class=line]'),
    ];
    const firstLineIndex = allLines.findIndex((node) => node === startLine);
    const lastLineIndex = allLines.findIndex((node) => node === endLine);
    const selectedLines = allLines.filter((_node, index) => index >= firstLineIndex && index <= lastLineIndex);
    const isStartAndEndSameNode = selectedLines.length === 1;
    const fragmentOf = (line) => line.querySelector('.fragment.mod-annotations-on');
    const textContentOf = (line) => fragmentOf(line).textContent;
    const highlight = (line, start, end) => {
        ...
        fragmentOf(line).innerHTML = [
            ...
            `<mark style="background:orange; color:black;">${textContent.slice(start, end)}</mark>`,
    ...
    if (isStartAndEndSameNode) {
        ...
        highlight(selectedLines[0], selectionStart.offset, textContentOf(selectedLines[0]).length);
        selectedLines
            .slice(1, -1)
