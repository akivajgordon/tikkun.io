...
const Book = (book) => `
  <li class="parsha-book">
    <ol class="parsha-list">
      ${book
    .map((p) => Parsha({ idx: p.idx, token: 'torah', he: p.he, key: slugify(p.en) }))
    .join('')}
    </ol>
  </li>
`;
const parshaFromLabel = ({ label }) => parshiyot.find(({ he }) => label.startsWith(he));
...
const Browse = () => `
  <div class="browse">
    ...
    books[book].push(Object.assign(Object.assign({}, parsha), { idx }));
    return books;
}, [])
    .map(Book)
    .join('')}
...
`;
const searchables = [
    ...parshiyot.map((p, index) => (Object.assign(Object.assign({ idx: index, token: 'torah' }, p), { key: slugify(p.en) }))),
    {
        idx: 'esther',
        token: 'esther',
        he: 'אסתר',
  ...
  `);
    scheduleFetcher.fetch().then((readingSchedule) => {
        const comingUpReadings = readingSchedule
            ...
            .slice(0, 3);
        const comingUpReadingsList = document.querySelector('#coming-up-readings-list');
        comingUpReadingsList.replaceWith(htmlToElement(`
        <ol id="coming-up-readings-list" class="cluster" style="list-style: none; display: table; margin-left: auto; margin-right: auto; white-space: nowrap;">
            ...
            comingUpReading.addEventListener('click', (e) => {
                ...
                const { ref, key } = {
                    torah: (idx) => {
                        ...
                        const parsha = parshaFromLabel({ label });
                        return {
                            ref: Object.assign(Object.assign({}, parsha.ref), { scroll: 'torah' }),
                            key: idx === 0 ? 'next' : slugify(parsha.en),
                        };
                    },
                    holydays: (idx) => ({
                        ref: Object.assign(Object.assign({}, holydays[idx].ref), { scroll: idx }),
                        key: idx,
                    }),
                    esther: () => ({
...
//# sourceMappingURL=ParshaPicker.js.map