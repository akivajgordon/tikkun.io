...
`,wc=t=>(e,a)=>a<t,Sc=(t,e)=>{const a=br(t,e,n=>[n.date.title.he,n.date.title.en]);return a.length?a.filter(wc(5)).map(n=>dc(n)):[hc()]};function kr(t,e){if(t.id===q.Megillah)return P.gettext(an(t.runs[0].scroll),"he-x-nonikud");let a=t.date.title.he.replace("פרשת ","");return t.id!==q.Shacharis&&(a+=`: ${t.id}`),!(e!=null&&e.forCalendar)&&a.startsWith("ראש חודש")?"ראש חודש":a}const _r=t=>{const e=t.forEntireChumash(new p).flatMap(i=>i.leinings),a=tc.new(),n=gc({search:Sc.bind(null,e),emitter:a}),o=e.filter(i=>i.date.date>new Date).slice(0,3),r=kc(`
  ...
  `,n.querySelector(".map-popover-close").addEventListener("click",()=>n.classList.remove("is-visible")),n.addEventListener("click",c=>{c.target===n&&n.classList.remove("is-visible")}),document.body.appendChild(n);let r=new p().getFullYear();const i=(c,d)=>{const g=n.querySelector(".map-popover-title");g.textContent=W(d);const _=n.querySelector(".map-popover-meta");_.innerHTML=`
  ...
  `,document.head.appendChild(e);const a=document.createElement("div");a.className="parsha-picker",a.style.display="flex",a.style.flexDirection="column";let n=new p().getFullYear();const o=()=>{a.innerHTML="";const r=document.createElement("div");r.className="miller-year-controls";const i=document.createElement("button");i.className="miller-year-btn",i.textContent=`« ${n-1}`,i.addEventListener("click",()=>{n--,o()});const l=document.createElement("span");l.textContent=`Cascading Columns — Year ${n}`,l.style.fontWeight="bold";const c=document.createElement("button");c.className="miller-year-btn",c.textContent=`${n+1} »`,c.addEventListener("click",()=>{n++,o()}),r.appendChild(i),r.appendChild(l),r.appendChild(c),a.appendChild(r);const d=document.createElement("div");d.className="miller-container";const g=document.createElement("div");g.className="miller-column";const _=document.createElement("div");_.className="miller-column";const f=document.createElement("div");f.className="miller-column";const E=document.createElement("div");E.className="miller-column",d.appendChild(g),d.appendChild(_),d.appendChild(f),d.appendChild(E),a.appendChild(d);const C=t.forHebrewYear(n),m={1:[],2:[],3:[],4:[],5:[]},I={"ימים נוראים":[],"סוכות ושמיני עצרת":[],"חנוכה ופורים":[],פסח:[],שבועות:[],תעניות:[],"ראש חודש":[]},w=[];C.forEach(v=>{v.leinings.forEach(k=>{if(k.isParsha){const u=k.runs[0].aliyot[0].start.b;m[u]&&m[u].push({date:v,inst:k})}else if(k.id===q.Megillah)w.push({date:v,inst:k});else{const u=k.date.title.he;u.includes("ראש השנה")||u.includes("כיפור")||u.includes("כפור")?I["ימים נוראים"].push({date:v,inst:k}):u.includes("סוכות")||u.includes("שמיני עצרת")||u.includes("שמחת תורה")||u.includes("הושענא רבא")?I["סוכות ושמיני עצרת"].push({date:v,inst:k}):u.includes("חנוכה")||u.includes("פורים")||u.includes("שבת שקלים")||u.includes("שבת זכור")?I["חנוכה ופורים"].push({date:v,inst:k}):u.includes("פסח")||u.includes("שבת הגדול")||u.includes("שבת פרה")||u.includes("שבת החודש")?I.פסח.push({date:v,inst:k}):u.includes("שבועות")?I.שבועות.push({date:v,inst:k}):u.includes("תענית")||u.includes("עשרה בטבת")||u.includes("שבעה עשר")||u.includes("איכה")||u.includes("צום")||u.includes("תשעה באב")?I.תעניות.push({date:v,inst:k}):I["ראש חודש"].push({date:v,inst:k})}})});const A=(v,k)=>{E.innerHTML="";const u=document.createElement("div");u.className="miller-details";const M=document.createElement("div");M.className="miller-details-title",M.textContent=W(k);const T=document.createElement("div");T.className="miller-details-meta",T.innerHTML=`
      ...
      `;const L=document.createElement("div");L.className="miller-details-actions",Se(k).forEach(R=>{const $=document.createElement("a");$.href=R.url,$.className=`miller-action-btn ${R.type!==B.Main?"mod-secondary":""}`,$.textContent=R.label,L.appendChild($)}),u.appendChild(M),u.appendChild(T),u.appendChild(L),E.appendChild(u)},b=()=>{E.innerHTML='<div class="miller-details-placeholder">בחר פריט כדי להציג פרטים</div>'},y=v=>{f.innerHTML="",b();const k=document.createElement("ul");k.className="miller-list",v.forEach(({date:u,inst:M})=>{const T=document.createElement("li");T.className="miller-item",T.textContent=W(M),T.addEventListener("click",()=>{f.querySelectorAll(".miller-item").forEach(L=>L.classList.remove("is-selected")),T.classList.add("is-selected"),A(u,M)}),k.appendChild(T)}),f.appendChild(k)},S=v=>{_.innerHTML="",f.innerHTML="",b();const k=document.createElement("ul");if(k.className="miller-list",v==="torah")[{id:1,name:"ספר בראשית"},{id:2,name:"ספר שמות"},{id:3,name:"ספר ויקרא"},{id:4,name:"ספר במדבר"},{id:5,name:"ספר דברים"}].forEach(M=>{const T=document.createElement("li");T.className="miller-item",T.innerHTML=`<span>${M.name}</span><span class="miller-item-arrow">◀</span>`,T.addEventListener("click",()=>{_.querySelectorAll(".miller-item").forEach(L=>L.classList.remove("is-selected")),T.classList.add("is-selected"),y(m[M.id])}),k.appendChild(T)});else if(v==="holiday")Object.keys(I).forEach(u=>{const M=I[u];if(!M.length)return;const T=document.createElement("li");T.className="miller-item",T.innerHTML=`<span>${u}</span><span class="miller-item-arrow">◀</span>`,T.addEventListener("click",()=>{_.querySelectorAll(".miller-item").forEach(L=>L.classList.remove("is-selected")),T.classList.add("is-selected"),y(M)}),k.appendChild(T)});else if(v==="megillah"){const u=document.createElement("li");u.className="miller-item",u.innerHTML='<span>מגילות המועדים</span><span class="miller-item-arrow">◀</span>',u.addEventListener("click",()=>{_.querySelectorAll(".miller-item").forEach(M=>M.classList.remove("is-selected")),u.classList.add("is-selected"),y(w)}),k.appendChild(u)}_.appendChild(k)},h=[{id:"torah",name:"תורה וחומשים"},{id:"holiday",name:"מועדים וחגים"},{id:"megillah",name:"מגילות המועדים"}],x=document.createElement("ul");x.className="miller-list",h.forEach(v=>{const k=document.createElement("li");k.className="miller-item",k.innerHTML=`<span>${v.name}</span><span class="miller-item-arrow">◀</span>`,k.addEventListener("click",()=>{g.querySelectorAll(".miller-item").forEach(u=>u.classList.remove("is-selected")),k.classList.add("is-selected"),S(v.id)}),x.appendChild(k)}),g.appendChild(x),b()};return o(),{node:a,onMount:()=>{console.log("Miller Columns Mounted")},onDestroy:()=>{e.remove()}}},zc=t=>{const e=document.createElement("style");e.textContent=`
  ...
  `,document.head.appendChild(e);const a=document.createElement("div");a.className="parsha-picker",a.style.display="flex",a.style.flexDirection="column";let n=new p().getFullYear(),o=null;const r={7:"תשרי",8:"חשון",9:"כסלו",10:"טבת",11:"שבט",12:"אדר",13:"אדר ב׳",1:"ניסן",2:"אייר",3:"סיון",4:"תמוז",5:"אב",6:"אלול"},i=[7,8,9,10,11,12,13,1,2,3,4,5,6],l=()=>{a.innerHTML="";const c=document.createElement("div");c.className="kanban-year-controls";const d=document.createElement("button");d.className="kanban-year-btn",d.textContent=`« ${n-1}`,d.addEventListener("click",()=>{n--,o=null,l()});const g=document.createElement("span");g.textContent=`Kanban Month Board — Year ${n}`,g.style.fontWeight="bold";const _=document.createElement("button");_.className="kanban-year-btn",_.textContent=`${n+1} »`,_.addEventListener("click",()=>{n++,o=null,l()}),c.appendChild(d),c.appendChild(g),c.appendChild(_),a.appendChild(c);const f=document.createElement("div");f.className="kanban-picker-container";const E=document.createElement("div");E.className="kanban-board";const C=document.createElement("div");C.className="kanban-drawer",C.innerHTML=`
    ...
    `,f.appendChild(E),f.appendChild(C),a.appendChild(f),C.querySelector("#drawer-close").addEventListener("click",()=>{C.classList.remove("is-open"),E.querySelectorAll(".kanban-card").forEach(b=>b.classList.remove("is-selected")),o=null});const I=t.forHebrewYear(n),w={};i.forEach(b=>{w[b]=[]}),I.forEach(b=>{b.leinings.forEach(y=>{const h=new p(b.date).getMonth();w[h]&&w[h].push({date:b,inst:y})})}),i.forEach(b=>{const y=w[b];if(!y||y.length===0&&b===13)return;const S=document.createElement("div");S.className="kanban-column";const h=document.createElement("div");h.className="kanban-column-header",h.innerHTML=`
        <span>${r[b]}</span>
        <span class="kanban-column-badge">${y.length} קריאות</span>
      `;const x=document.createElement("div");x.className="kanban-cards-list",y.forEach(({date:v,inst:k})=>{const u=document.createElement("div"),M=`${v.id}:${k.id}`;u.className=`kanban-card ${o===M?"is-selected":""}`;const T=!k.isParsha,L=T?"חג/מועד":"פרשה",O=T?"kanban-card-tag mod-holiday":"kanban-card-tag";u.innerHTML=`
          <div class="kanban-card-title">${W(k)}</div>
          <div class="kanban-card-date">${re(v.date)}</div>
          <div class="${O}">${L}</div>
        `,u.addEventListener("click",()=>{E.querySelectorAll(".kanban-card").forEach(R=>R.classList.remove("is-selected")),u.classList.add("is-selected"),o=M,A(v,k)}),x.appendChild(u)}),S.appendChild(h),S.appendChild(x),E.appendChild(S)});const A=(b,y)=>{const S=C.querySelector("#drawer-title"),h=C.querySelector("#drawer-body");S.textContent=W(y),h.innerHTML=`
      ...
      `;const x=h.querySelector("#drawer-actions");Se(y).forEach(k=>{const u=document.createElement("a");u.href=k.url,u.className=`kanban-action-btn ${k.type!==B.Main?"mod-secondary":""}`,u.textContent=k.label,x.appendChild(u)}),C.classList.add("is-open")};if(o){const[b,y]=o.split(":"),S=I.flatMap(h=>h.leinings.map(x=>({date:h,inst:x}))).find(h=>h.date.id===b&&h.inst.id===y);S&&A(S.date,S.inst)}};return l(),{node:a,onMount:()=>{console.log("Kanban Board Mounted")},onDestroy:()=>{e.remove()}}},Yc=t=>{const e=document.createElement("style");e.textContent=`
    .terminal-container {
