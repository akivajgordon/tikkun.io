(() => {
  ...
  var fetchPage = ({ path, title, pageNumber }) => window.fetch(path).then((res) => res.json()).then((page) => ({ content: page, title, pageNumber })).catch((err) => {
    console.error(err);
  });
  var aliyotStrings = [
    "\u05E8\u05D0\u05E9\u05D5\u05DF",
  ...
  var Scroll = {
    new: ({
      ...
      const iterator = integer_iterator_default.new({ startingAt: pageNumber });
      return {
        scrollName: scroll2,
        fetchPrevious: () => {
        ...
        aliyotFor: ({ verses }) => {
          ...
          const display = (aliyah) => {
            return aliyahName({
              aliyah,
              getParshaName: () => {
                const found2 = aliyahFinder.find(
                  ({ ref }) => verses.some(
                    ({ book: b, chapter: c, verse: v }) => ref.b === b && ref.c === c && ref.v === v
                  )
                );
                return found2?.he;
              }
  ...
  var scrolls_by_key_default = {
    torah: TorahScroll,
    esther: EstherScroll,
    ...Object.keys(holydays_default).reduce((result, holydayKey) => {
      const HolydayScroll = {
        new: ({ startingAtRef }) => {
          return Scroll.new({
            scroll: holydayKey,
  ...
  var hebrew_numeral_default = hebrewNumeral;

  // src/text-filter.js
  var NUN_HAFUCHA = "\u05C6";
  var ketiv = (text) => text.replace("#(\u05E4)", "").replace(`(${NUN_HAFUCHA})#`, `${NUN_HAFUCHA} `).replace(`#(${NUN_HAFUCHA})`, ` ${NUN_HAFUCHA}`).split(" ").map(
    (maqafSeparatedWord) => maqafSeparatedWord.split("\u05BE").map((word) => {
  ...
  var text_filter_default = ({ text, annotated }) => annotated ? ketiv(text) : kri(text);
  ...
  var asVersesRange = (verses) => asRange(
    verses.map((verse) => {
      const components = [];
      if (verse.verse === 1) {
      ...
      return components.map((num) => hebrew_numeral_default(num)).join(":");
    })
  );
  var display_range_default = { asVersesRange };

  // src/title.js
  var title_default = (parshiyot) => {
  ...
  // src/location.js
  var tocFromScroll = {
    torah: table_of_contents_default,
    esther: table_of_contents_esther_default,
    ...table_of_contents_holydays_default
  };
  var defaultRef = () => {
  ...
  var resolveToValidRef = ({
    scroll: scroll2 = "torah",
    book,
    chapter,
    verse
  }) => {
  ...
  var ParshaRouter = {
    refFromPathParts: ({ pathParts }) => {
      ...
      const found = parshiyot_default.find(({ he, en }) => {
  ...
  var NextRouter = {
    refFromPathParts: ({ pathParts, asOfDate }) => {
      const { label } = schedule_default.find(
        ({ datetime }) => new Date(datetime) > new Date(asOfDate || Date.now())
      );
      const found = parshiyot_default.find(({ he }) => label.split("\u2013")[0].trim() === he);
      const { b, c, v } = found.ref;
      return { scroll: "torah", b, c, v };
    }
  };
  var DefaultRouter = { refFromPathParts: () => defaultRef() };
  var url_to_ref_default = ({ url, asOfDate }) => {
  ...
  <tr>
    ...
    (column) => `
      ...
      (fragment) => `
        ...
        text_filter_default({ text: fragment, annotated: true })
      )}</span>
            <span class="fragment ${setumaClass(
        column
      )} mod-annotations-off">${ktivKriAnnotation(
        text_filter_default({ text: fragment, annotated: false })
      )}</span>
          `
    ).join("")}
        </div>
      ...
      <span class="location-indicator mod-verses">${display_range_default.asVersesRange(
    verses
  )}</span>
...
`;
  ...
  var bestMatch = (needle, getSearchTerms) => (candidate) => {
    const { minScore, index } = getSearchTerms(candidate).map(
      (term) => hasEveryCharacterInOrder(needle)(term) ? indexScore(needle, term) : Infinity
    ).reduce(
      ({ minScore: minScore2, index: index2 }, score, i) => {
        if (score < minScore2)
  ...
  var fuzzy_default = (haystack, needle, getSearchTerms = (x) => [x]) => haystack.map(bestMatch(needle, getSearchTerms)).filter(({ score }) => isFinite(score)).sort((match, other) => {
...
`);
  ...
  <li class="parsha-book">
    <ol class="parsha-list">
      ${book.map((p) => Parsha({ idx: p.idx, token: "torah", he: p.he, key: slugify_default(p.en) })).join("")}
    </ol>
  </li>
`;
  var parshaFromLabel = ({ label }) => parshiyot_default.find(({ he }) => label.startsWith(he));
  ...
  <li style="display: table-cell; width: calc(100% / 3); padding: 0 0.5em;">
    <div class="stack small" style="display: flex; flex-direction: column; align-items: center;">
      <button
        ...
        data-idx="${index}"
        data-key="${index === 0 ? "next" : slugify_default(parsha.en)}"
...
`;
  ...
  <div class="browse">
    ...
    <ol class="parsha-books mod-emphasize-first-in-group">
      ${parshiyot_default.reduce((books, parsha, idx) => {
    ...
    <ol class="parsha-books">
      ${holydaysLayout.map((col) => `
        <li class="parsha-book">
          <ol class="parsha-list">
            ${col.map((holydayKey) => {
    const holyday = holydays_default[holydayKey];
...
`;
  var searchables = [
    ...parshiyot_default.map((p, index) => ({ idx: index, token: "torah", ...p, key: slugify_default(p.en) })),
    {
      idx: "esther",
      token: "esther",
      he: "\u05D0\u05E1\u05EA\u05E8",
    ...
    ...Object.keys(holydays_default).map((holydayKey) => {
      const holyday = holydays_default[holydayKey];
      const { he, en } = holyday;
      return {
        idx: holydayKey,
  ...
  var searchResults = (query) => {
    const results = fuzzy_default(searchables, query, (parsha) => [parsha.he, parsha.en]);
  ...
  var top = (n) => (_, i) => i < n;
  var search = (query) => searchResults(query).filter(top(5)).map(
    (result) => result.item === "No results" ? NoResults() : ParshaResult_default(result)
  );
  ...
  `);
    searchEmitter.on("selection", (selected) => {
      ...
      const { ref, key } = {
        torah: (idx2) => {
          const parsha = parshiyot_default[Number(idx2)];
          return { ref: { ...parsha.ref, scroll: "torah" }, key: slugify_default(parsha.en) };
        },
        holydays: (idx2) => ({ ref: { ...holydays_default[idx2].ref, scroll: idx2 }, key: idx2 }),
    ...
    [...self.querySelectorAll('[data-target-id="parsha"]')].forEach((parsha) => {
      parsha.addEventListener("click", (e) => {
        ...
        const { ref, key } = {
          torah: (idx2) => {
            const parsha2 = parshiyot_default[Number(idx2)];
            return { ref: { ...parsha2.ref, scroll: "torah" }, key: slugify_default(parsha2.en) };
          },
          holydays: (idx2) => ({ ref: { ...holydays_default[idx2].ref, scroll: idx2 }, key: idx2 }),
          esther: () => ({ ref: { b: 1, c: 1, v: 1, scroll: "esther" }, key: "esther" })
        }[token](idx);
      ...
      comingUpReading.addEventListener("click", (e) => {
        ...
        const { ref, key } = {
          torah: (idx2) => {
            ...
            const parsha = parshaFromLabel({ label });
            return { ref: { ...parsha.ref, scroll: "torah" }, key: idx2 === 0 ? "next" : slugify_default(parsha.en) };
          },
          holydays: (idx2) => ({ ref: { ...holydays_default[idx2].ref, scroll: idx2 }, key: idx2 }),
          esther: () => ({ ref: { b: 1, c: 1, v: 1, scroll: "esther" }, key: "esther" })
        }[token](idx);
  ...
  var makePageNode = ({ title, pageNumber }) => {
  ...
  var renderPage = ({ insertStrategy: insert }) => ({ content, title, pageNumber }) => {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    toggle.addEventListener(
      "change",
      (e) => toggleAnnotations(() => !e.target.checked)
    );
    document.addEventListener(
      "keydown",
      whenKey3("Shift", () => toggleAnnotations(() => toggle.checked))
    );
    document.addEventListener(
      "keyup",
      whenKey3("Shift", () => toggleAnnotations(() => toggle.checked))
    ...
    const startingRef = url_to_ref_default({ url: window.location.href });
