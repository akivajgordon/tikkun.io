...
test('prioritizes matches that are closer together (all same-length strings)', t => {
  ...
  t.deepEqual(results.map(r => r.item), matches)
})
...
test('returns the match indexes', t => {
  ...
  t.deepEqual(results.map(({ match }) => match.indexes), [
    [2, 3, 5, 6],
    [0, 2, 4, 5]
  ])
})
...
test('searches case insensitively', t => {
  ...
  t.deepEqual(fuzzy(haystack, 'abc').map(r => r.item), ['Abcd', 'a_bcd'])
  t.deepEqual(fuzzy(haystack, 'ABc').map(r => r.item), ['Abcd', 'a_bcd'])
})
...
test('searches multiple acceptable forms and matches the best form', t => {
  ...
  t.deepEqual(results.map(r => r.match), [
    { index: 0, indexes: [0, 1] },
    { index: 1, indexes: [1, 2] }
  ])
})
